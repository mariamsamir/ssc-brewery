...
public class BeerOrderControllerTest extends BaseIT {
    ...
    @Test
    void createOrderNotAuthorized() throws Exception {
    ...
    void createOrderUserNotAuthCustomer() throws Exception {
        BeerOrderDto beerOrderDto = buildOrderDto(keyWestCustomer, loadedBeers.get(0).getId());

        mockMvc.perform(post(API_ROOT + keyWestCustomer.getId() + "/orders")
                        .accept(MediaType.APPLICATION_JSON)
                        .characterEncoding("UTF-8")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(beerOrderDto)))
